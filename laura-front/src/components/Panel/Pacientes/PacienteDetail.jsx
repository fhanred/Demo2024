...
const PacienteDetail = ({pacienteId, back}) => {
    ...
    useEffect(() => {
      ...
      const deleteClient = async () => {
        const {data} = await axios.delete("/client/"+paciente.id)
        back()
      }


      if(edit) return <PacienteEdit user={paciente} back={() => {setEdit(false); reloadUser()}}/>
    ...
    return (
        ...
        <br></br><br></br><br></br>
          ...
          { JSON.parse(localStorage.getItem("user"))?.role >= 1 && <><p><b>Cedula:</b> {paciente ? paciente.cedula : <span style={{color:"grey"}}>Cargando..</span>}</p>
          ...
          <div className={style.column}>
          { JSON.parse(localStorage.getItem("user"))?.role >= 1 && <><p><b>Direccion:</b> {paciente ? paciente.direccion : <span style={{color:"grey"}}>Cargando..</span>}</p>
        ...
        <br></br>
        {JSON.parse(localStorage.getItem("user")).role >= 3 &&<><button className={style.button} onClick={() => confirmAlert(options)}>Eliminar cliente</button><br></br><br></br></>}
        {JSON.parse(localStorage.getItem("user")).role >= 2 && <><button className={style.button} onClick={() => setEdit(true)}>Editar cliente</button><br></br><br></br></>}
